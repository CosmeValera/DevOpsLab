...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [usersRes, projectsRes, deploymentsRes, metricsRes] =
          await Promise.all([
        ...
        const [usersData, projectsData, deploymentsData, metricsData] =
          await Promise.all([
            usersRes.json(),
            projectsRes.json(),
            deploymentsRes.json(),
          ...
          {metrics.slice(0, 6).map((metric, index) => (
            <div
              ...
              <div
                ...
                {metric.service_name === "backend" && (
                  <Server style={{ width: "16px", height: "16px" }} />
                )}
                {metric.service_name === "database" && (
